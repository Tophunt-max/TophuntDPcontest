...
/**
 * Testing kill switch: skip every `CACHE_KV` write.
 *
 ...
 * `KV_WRITES_DISABLED = "true"` lets a solo test session run without spending
 * the quota at all: every cache read simply misses and the value is recomputed
 * from D1, which is the fail-open path this module is already built around.
 *
 ...
 *   - Rate limiting honours it only for fail-OPEN keys (see lib/rateLimit.ts).
 *     Money and credential guards keep writing.
 *
