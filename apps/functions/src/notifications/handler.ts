...
export const notify = onCall({
    ...
    if (!type || !recipientId) {
        throw new HttpsError("invalid-argument", "Missing required fields: type, recipientId.");
    }
    ...
    try {
        ...
        switch (type) {
            ...
            case "COMMENT":
                ...
                body = `${data.actorName || "Someone"} ${data.isReply ? 'replied' : 'commented'}: "${data.text?.substring(0, 50)}"`;
                break;
        ...
        throw new HttpsError("internal", error.message || "Notification delivery failed.");
    }
});
