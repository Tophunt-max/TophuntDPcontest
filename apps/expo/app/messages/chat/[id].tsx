...
import { View, Text, ActivityIndicator, StyleSheet, TouchableOpacity, Platform, Alert, TextInput, Modal, Pressable, KeyboardAvoidingView } from 'react-native';
...
export default function ChatScreen() {
  ...
  useEffect(() => {
    ...
    const unsubscribeMessages = onSnapshot(q, (snapshot) => {
      const loadedMessages = snapshot.docs.map(doc => {
        ...
        return {
          _id: String(doc.id),
          text: displayTitle || String(data.content || ''),
          createdAt: msgTime,
          user: { _id: String(data.senderId) },
          image: data.type === 'image' ? String(data.content) : undefined,
          audio: data.type === 'voice_note' ? String(data.content) : undefined,
          video: data.type === 'video_message' ? String(data.content) : undefined,
          system: !!displayTitle,
  ...
  const startRecording = async () => {
    ...
    try {
      ...
      if (permission.status === 'granted') {
        ...
        setMeteringData([]);
        setIsRecording(true);
        recording.setOnRecordingStatusUpdate((s) => { if (s.metering !== undefined) setMeteringData(prev => [...prev, status.metering!]); });
  ...
  const renderBubble = useCallback((props: BubbleProps<IMessage>) => (
    <TouchableOpacity activeOpacity={0.8} onLongPress={() => handleMsgAction(props.currentMessage!)} onPress={() => Platform.OS === 'web' && handleMsgAction(props.currentMessage!)}>
        <Bubble {...props} renderTime={() => null} wrapperStyle={{ right: { backgroundColor: '#FF4D67', borderRadius: 20, borderBottomRightRadius: 5, padding: 5, marginVertical: 4 }, left: { backgroundColor: '#F3F3F5', borderRadius: 20, borderBottomLeftRadius: 5, padding: 5, marginVertical: 4 } }} textStyle={{ right: { color: '#fff', fontSize: 16, fontFamily: 'Urbanist-Medium' }, left: { color: '#000', fontSize: 16, fontFamily: 'Urbanist-Medium' } }} renderTicks={() => props.currentMessage?.user._id === currentUser?.uid && (props.currentMessage?.seen ? <Checkmark_Double width={16} height={16} color="#34B7F1" style={{ marginLeft: 4 }} /> : <Checkmark_Single width={16} height={16} color="rgba(255,255,255,0.6)" style={{ marginLeft: 4 }} />)} />
  ...
  const CustomComposer = useCallback((props: any) => {
      return (
        <View style={styles.composerWrapper}>
            <TouchableOpacity style={styles.actionButton} onPress={() => setShowEmojiPicker(!showEmojiPicker)}>
                <Emoji_Icon width={24} height={24} />
            </TouchableOpacity>
            <View style={styles.inputContainer}>
                {isRecording ? <RecordingWaveform meteringData={meteringData} /> : 
                <TextInput 
                    ref={inputRef}
                    ...
                    value={inputText} 
                    onChangeText={(t) => { setInputText(t); setTypingStatus(chatId as string, t.length > 0); }} 
                    multiline 
                />}
            </View>
            <TouchableOpacity style={styles.actionButton} onPress={async () => {
                ...
                <Gallery_Icon width={24} height={24} />
            </TouchableOpacity>
        </View>
      );
  }, [isRecording, meteringData, inputText, showEmojiPicker, chatId]);
  ...
  const renderInputToolbar = useCallback((props: InputToolbarProps<IMessage>) => {
    ...
    return (
        <InputToolbar {...props} containerStyle={styles.inputToolbar} renderComposer={CustomComposer}
          renderSend={() => (
  ...
  }, [didIBlock, amIBlocked, inputText, isRecording, CustomComposer, onSend, recipient.uid, chatId]);
  ...
  return (
    <View style={{ flex: 1, backgroundColor: '#FFFFFF' }}>
      <ChatHeader recipientName={String(recipient.name)} recipientAvatar={String(recipient.avatar)} status={isTyping ? 'typing...' : String(otherUserStatus)} onAudioCall={() => !amIBlocked && !didIBlock && initiateCall(chatId as string, recipient.uid, 'audio', { sdp: 'init' })} onVideoCall={() => !amIBlocked && !didIBlock && initiateCall(chatId as string, recipient.uid, 'video', { sdp: 'init' })} onSearchPress={() => setIsSearchVisible(!isSearchVisible)} onOptionsPress={() => {}} />
      ...
      >
        <GiftedChat 
            ...
            renderMessageAudio={(p) => <View style={{ padding: 5, minWidth: 150 }}><AudioPlayer uri={p.currentMessage.audio} isSender={p.currentMessage.user._id === currentUser?.uid} /></View>} 
            ...
            // Logic to keep input above keyboard
            bottomOffset={Platform.OS === 'ios' ? 0 : 0}
            keyboardShouldPersistTaps='never'
        />
      </KeyboardAvoidingView>

      {showEmojiPicker && <View style={styles.emojiContainer}>{EMOJIS.map(e => (<TouchableOpacity key={e} onPress={() => { setInputText(p => p + e); setTypingStatus(chatId as string, true); }} style={styles.emojiItem}><Text style={{ fontSize: 24 }}>{e}</Text></TouchableOpacity>))}</View>}
...
const styles = StyleSheet.create({
  ...
  micButton: { backgroundColor: '#FF4D67', width: 46, height: 46, borderRadius: 23, justifyContent: 'center', alignItems: 'center', marginLeft: 10 },
  systemMessageContainer: { alignItems: 'center', justifyContent: 'center', marginVertical: 8 },
  systemMessagePill: { backgroundColor: '#F5F5F5', paddingHorizontal: 12, paddingVertical: 6, borderRadius: 15 },
  systemMessageText: { fontSize: 12, color: '#666', fontFamily: 'Urbanist-Medium' },
  modalOverlay: { flex: 1, backgroundColor: 'rgba(0,0,0,0.5)', justifyContent: 'center', alignItems: 'center' },
  modalContent: { backgroundColor: 'white', width: '80%', borderRadius: 20, padding: 20, shadowColor: '#000', shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.25, shadowRadius: 4, elevation: 5 },
