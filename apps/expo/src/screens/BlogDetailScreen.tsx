...
/**
 * Shared blog post screen. Rendered by BOTH:
 *   - /blog/[slug]      (in-app navigation)
 *   - /[slug]           (root-level permalinks matching the original tophunt.in
 *                        WordPress URLs, e.g. /amazon-cadbury-vday-quiz-answers)
 * The post is resolved from the `slug` route param either way.
 */
export default function BlogDetailScreen() {
  ...
  const goToBlog = () => {
    // router.back() is unreliable on a direct/deep link (no history) — send the
    // visitor to the blog listing instead.
    try {
      router.replace('/blog');
    } catch {
      router.replace('/');
    ...
    <SafeAreaView style={[styles.container, { backgroundColor }]}>
      ...
      {loading ? (
        ...
        <View style={styles.center}>
          ...
          <TouchableOpacity onPress={goToBlog} style={{ marginTop: 16 }}>
            <Text style={{ color: primaryColor, fontWeight: '700' }}>Browse the blog</Text>
          </TouchableOpacity>
        </View>
      ) : (
